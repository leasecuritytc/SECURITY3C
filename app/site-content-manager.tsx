...
const blankPortfolio: PortfolioDraft = { title: "", area: "", serviceType: "", serviceMode: "Instalação", city: "", completedAt: "", summary: "", altText: "", status: "Rascunho", sortOrder: 0, featured: false };

export function SiteContentManager({ section, areaFilter, serviceTypeFilter, onAuditChanged }: { section: "portfolio" | "texts"; areaFilter?: string; serviceTypeFilter?: string; onAuditChanged?: () => Promise<void> | void }) {
  ...
  const visibleItems = items.filter((item) => (!areaFilter || item.area === areaFilter) && (!serviceTypeFilter || item.serviceType === serviceTypeFilter));
  const filterLabel = areaFilter ? `Área: ${areaFilter}` : serviceTypeFilter ? `Serviço: ${serviceTypeFilter}` : "Todos os serviços";
  const newPortfolioDraft = () => ({ ...blankPortfolio, area: areaFilter || "", serviceType: serviceTypeFilter || "" });
  ...
  return <>
    <div className="page-heading"><div><span className="eyebrow">Conteúdo exclusivo</span><h1>{section === "portfolio" ? "Serviços realizados" : "Textos da página pública"}</h1><p>{section === "portfolio" ? "Insira, substitua, organize e publique somente imagens e informações de serviços efetivamente realizados." : "Somente a conta administrativa autorizada pode editar os textos institucionais da landing page."}</p></div><Button variant="outline" asChild><a href="/" target="_blank"><Eye size={17} /> Visualizar página <ExternalLink size={14} /></a></Button></div>
    {section === "portfolio" ? <section className="panel page-panel">
      <div className="panel-head"><div><span className="eyebrow">{filterLabel}</span><h2>Portfólio de serviços reais</h2><p>Cadastre trabalhos executados e imagens autorizadas. Use o menu lateral para filtrar por área ou tipo de serviço.</p></div><Button className="primary-action" onClick={() => setEditor(newPortfolioDraft())}><Plus size={17} /> Novo serviço realizado</Button></div>
      {loading ? <div className="cms-loading"><Loader2 className="animate-spin" /> Carregando conteúdo...</div> : visibleItems.length === 0 ? <div className="cms-empty"><ImagePlus /><strong>Nenhum serviço neste filtro</strong><p>O carrossel público não exibirá trabalhos fictícios. Cadastre o primeiro serviço real desta categoria.</p><Button onClick={() => setEditor(newPortfolioDraft())}><Plus size={16} /> Cadastrar serviço</Button></div> : <div className="portfolio-admin-grid">{visibleItems.map((item) => <article className="portfolio-admin-card" key={item.id}><div className="portfolio-admin-image"><img src={item.imageUrl} alt={item.altText} /><Badge className={item.status === "Publicado" ? "cms-status-published" : "cms-status-draft"}>{item.status}</Badge></div><div className="portfolio-admin-body"><div><small>{item.area} • {item.serviceMode} • {item.serviceType}</small><h3>{item.title}</h3><p>{item.summary}</p></div><div className="portfolio-admin-footer"><span>Ordem {item.sortOrder}{item.featured ? " • Destaque" : ""}</span><div><Button size="icon" variant="ghost" aria-label="Editar imagem e conteúdo" title="Editar imagem e conteúdo" onClick={() => setEditor({ ...item })}><Pencil size={16} /></Button><Button size="icon" variant="ghost" aria-label="Excluir" title="Excluir" onClick={() => setDeleteTarget(item)}><Trash2 size={16} /></Button></div></div></div></article>)}</div>}
    </section> : <section className="panel page-panel"><div className="panel-head"><div><span className="eyebrow">Editor institucional</span><h2>Textos da landing page</h2><p>Altere as mensagens principais sem editar o código. Os serviços oferecidos permanecem padronizados.</p></div><Button className="primary-action" onClick={() => void saveContent()} disabled={savingContent}>{savingContent ? <Loader2 className="animate-spin" /> : <Save size={17} />} Salvar textos</Button></div>
