...
import { ArrowDownRight, ArrowUpRight, CircleDollarSign, Download, FileCheck2, FileText, Pencil, Plus, ReceiptText, Search, ShieldCheck, Trash2, WalletCards } from "lucide-react";
...
export function FinanceManager({ rows, customers, query, loading, saving, onSave, onDelete }: {
  ...
  return <>
    ...
    <section className="finance-action-grid" aria-label="Ações rápidas">
      <button className="finance-action-card receipt" onClick={() => setDialog({ mode: "receipt" })}><span><ReceiptText /></span><div><strong>Recibo rápido</strong><small>Registre um pagamento e gere PDF ou Word.</small></div><b>→</b></button>
...
function FinanceDialog({ mode, record, customers, saving, onClose, onSave }: {
  ...
  return <Dialog open onOpenChange={(open) => !open && onClose()}><DialogContent className="sm:max-w-[850px] max-h-[92vh] overflow-y-auto"><form onSubmit={submit}><DialogHeader><DialogTitle>{title}</DialogTitle><DialogDescription>{receipt ? "O pagamento será registrado como receita paga e o recibo ficará disponível em PDF e Word." : "Informe dados reais. Toda inclusão ou alteração é registrada na auditoria."}</DialogDescription></DialogHeader>
